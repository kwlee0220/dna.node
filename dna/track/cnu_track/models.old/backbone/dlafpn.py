...
# from detectron2.layers import ModulatedDeformConv, Conv2d
...
class Tree(nn.Module):
    def __init__(self, cfg, levels, block, in_channels, out_channels, stride=1,
        ...
        if levels == 1:
            ...
            self.tree2 = Tree(cfg, levels - 1, block, out_channels,
                              out_channels,
...
class DLA(Backbone):
    def __init__(self, cfg, levels, channels, block=BasicBlock,
                 residual_root=False):
        ...
        self._out_feature_channels = {k: channels[i] for i, k in
                                      enumerate(self._out_features)}
        self._out_feature_strides = {k: 2 ** i for i, k in
                                     enumerate(self._out_features)}
...
class DLAUP(Backbone):
    def __init__(self, bottom_up, in_features, norm, dlaup_node='conv'):
        ...
        in_levels = [int(math.log2(input_shapes[f].stride)) for f in
                     in_features]
        self.in_features = in_features
        out_features = ['dlaup{}'.format(l) for l in in_levels]
        self._out_features = out_features
        self._out_feature_channels = {
            'dlaup{}'.format(l): in_channels[i] for i, l in
            enumerate(in_levels)}
